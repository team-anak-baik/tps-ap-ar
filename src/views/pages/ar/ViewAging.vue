<template>
          ...
          <table v-if="invoices && invoices.vendors" class="table table-hover table-striped table-bordered table-sm text-center align-middle">
            ...
            <tbody class="text-start">
              <tr v-for="(mnt, idx) in filteredData">
                ...
                <td>
                  {{ mnt.vendor.custmrcode }}
                </td>
                <td>
                  {{ mnt.vendor.custmrname }}
                </td>
                <td>
                ...
                <td class="text-center">
                  <CButtonGroup>
                    <CButton v-c-tooltip="{ content: 'Detail aging', placement: 'top' }" color="success" size="sm" @click="getDetail(mnt.vendor.custmrcode)">
                      <CIcon icon="cil-description" width="20" height="20" />
                    </CButton>
                    <CButton v-c-tooltip="{ content: 'Export to excel', placement: 'top' }" color="secondary" size="sm" @click="exportInvoicesToXLS(mnt.vendor.custmrcode)">
          ...
          <table v-if="invoice && invoice.vendor" hidden class="table-invoices">
            <tbody>
              ...
              <tr v-for="inv in invoice?.vendor.invoices.current">
                <td>{{ invoice?.vendor.custmrcode }}</td>
                <td>{{ invoice?.vendor.custmrname }}</td>
                <td>{{ inv.docnum }}</td>
          ...
          <table v-if="invoices && invoices.vendors" class="table table-hover table-striped table-bordered table-sm text-start align-middle">
            <tbody v-for="(mnt, idx) in invoices.vendors">
              <tr class="text-center table-danger">
                <th><strong>Customer Num.</strong></th>
              ...
              <tr v-for="invoice in mnt.vendor.invoices.current">
                <td>{{ mnt.vendor.custmrcode }}</td>
                <td>{{ mnt.vendor.custmrname }}</td>
  ...
  >
    <CModalHeader v-if="invoice && invoice.vendor">
      <CModalTitle>{{ invoice?.vendor.custmrcode }} | {{ invoice?.vendor.custmrname }} </CModalTitle>
    </CModalHeader>
    <CModalBody>
  ...
  computed: {
    filteredData() {
      ...
      const filtered = this.invoices.vendors.filter((item) => {
        if (item && (item.custmrname || item.custmrcode)) {
          const vendorName = item.custmrname ? item.custmrname.toLowerCase() : "";
          const vendorCode = item.custmrcode ? item.custmrcode.toLowerCase() : "";
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        this.$root.loader = true;
        const data = await ApiService.getData(this.filter);
        this.invoices = data;
        console.log(data);
      } catch (error) {
        this.getAlert("danger", "Error!", error.message);
    ...
    getAlert(color, title, message, timeout = 3000) {
